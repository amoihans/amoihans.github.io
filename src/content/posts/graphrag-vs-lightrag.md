...
**索引阶段** — 离线处理，将原始文档转化为层次化知识图谱：

```mermaid
flowchart TB
    A["📄 源文档"] --> B["✂️ 文本分块<br/>(~300 tokens/chunk)"]
    B --> C["🤖 LLM 实体/关系抽取<br/>(支持多次 gleaning 迭代)"]
    C --> D["🔗 图构建<br/>(实体→节点, 关系→边)"]
    D --> E["🧩 Leiden 社区检测<br/>(层次化聚类, 粗细粒度分层)"]
    E --> F["📝 社区摘要生成<br/>(LLM 逐层总结社区内容)"]
    F --> G["💾 持久化输出: Parquet 文件"]
```

> 索引阶段的 Parquet 输出是整个系统的知识基础——包含实体表、关系表、社区表、社区报告表，供查询阶段加载使用。

**查询阶段** — 在线响应，根据问题类型路由到不同检索策略：

```mermaid
flowchart TB
    H["❓ 用户查询"] --> I{"查询类型判断"}
    I -->|"实体/关系类问题"| J["🔍 Local Search<br/>实体向量匹配 + 邻居遍历"]
    I -->|"全局/主题类问题"| K["🌐 Global Search<br/>社区报告 Map-Reduce 综合"]
    I -->|"大规模图混合问题"| L["🧭 Drift Search<br/>局部起步, 逐步漂移到相关社区"]
    J --> M["🤖 LLM 结合检索上下文生成答案"]
    K --> M
    L --> M
```
...
**索引阶段** — 异步批处理，单次 LLM 调用完成抽取 + 并行写入：

```mermaid
flowchart TB
    A["📄 文档"] --> B["✂️ 文本分块"]
    B --> C["🤖 LLM 一次性抽取<br/>实体 + 关系 + 属性"]
    C --> D["🔍 余弦相似度去重<br/>(threshold=0.9, 合并同义实体)"]
    D --> E["📊 双层图谱构建"]
    E --> E1["低层图: 实体-关系网络<br/>(节点=实体, 边=关系)"]
    E --> E2["高层图: Leiden 社区结构<br/>(节点=社区, 边=社区关联)"]
    E1 --> F["📥 asyncio.gather 并行写入<br/>Semaphore 控制并发"]
    E2 --> F
    F --> G["💾 多后端存储<br/>JSON / Neo4j / PostgreSQL / MongoDB"]
```

> 索引阶段产出双层图结构（低层实体网络 + 高层社区结构），**不做社区摘要**——这是与 GraphRAG 最核心的架构差异。

**查询阶段** — 双层并行检索 + 动态聚合，无需预计算社区摘要：

```mermaid
flowchart TB
    H["❓ 用户查询"] --> I["🔑 关键词提取<br/>(实体词 + 主题词)"]
    I --> J["📡 双层并行检索"]
    J --> J1["低层检索<br/>实体匹配 → 图遍历 → 邻居扩展"]
    J --> J2["高层检索<br/>社区匹配 → 主题节点 → 跨实体聚合"]
    J1 --> K["🔄 多路召回融合<br/>(低层精确 + 高层全局)"]
    J2 --> K
    K --> L["📈 Rerank 重排序<br/>(过滤噪声, 提升 Top-K)"]
    L --> M["🤖 LLM 结合上下文生成答案"]
```
