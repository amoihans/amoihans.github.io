...
│   低层检索 (实体级)  │   高层检索 (主题级)               │
├─────────────────┼───────────────────────────────────┤
│ • 精确匹配实体名    │ • 基于 Leiden 社区结构              │
│ • 图遍历邻居扩展    │ • 关键词匹配社区摘要                 │
│ • 捕获直接关系      │ • 跨实体主题聚合                   │
│ • 适合: "张三是谁？" │ • 适合: "AI 发展趋势如何？"        │
└─────────────────┴───────────────────────────────────┘
```
...
```text
文本A: "乔布斯创立了苹果"  ──抽取──→  [乔布斯]  [苹果]
文本B: "Steve Jobs 领导 Apple" ──抽取──→  [Steve Jobs]  [Apple]
                                         ↓
                              余弦相似度(embedding) > 0.9
                                         ↓
                              合并为统一实体: [Steve Jobs/乔布斯]
```
...
<script>
// Mermaid diagram rendering — client-side via CDN
{
  const CODE_BLOCK_SELECTOR = 'pre > code[class*="language-mermaid"]';
    ...
    import("https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs")
      .then((mermaid) => {
        ...
        codeBlocks.forEach((code, i) => {
          ...
          container.setAttribute("data-mermaid-id", `mermaid-${i}`);
        });

        mermaid.default.run();
      })
      .catch((err) => console.warn("Mermaid CDN load failed:", err));
  }
